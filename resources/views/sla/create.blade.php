...
@section('titre', 'Créer un SLA')
@section('sous-titre', 'Définir un nouvel accord de niveau de service')

@section('contenu')
<div class="space-y-6">
    <!-- Header -->
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-white">Nouveau SLA</h1>
        <a href="{{ route('sla.index') }}" class="text-gray-400 hover:text-white transition">
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('sla.store') }}" class="max-w-2xl space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="nom" value="{{ old('nom') }}" 
                ...
                placeholder="Ex: SLA Premium - Critique">
            @error('nom')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="temps_reponse_heures" value="{{ old('temps_reponse_heures', 4) }}" step="0.25" min="0.25" max="168"
                ...
                @error('temps_reponse_heures')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-xs text-gray-400 mt-1">Délai avant première réponse</p>
            </div>
            ...
            <div>
                ...
                <input type="number" name="temps_resolution_heures" value="{{ old('temps_resolution_heures', 24) }}" step="0.25" min="0.25" max="720"
                ...
                @error('temps_resolution_heures')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-xs text-gray-400 mt-1">Délai de résolution complète</p>
            </div>
        </div>
        ...
        <div>
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="actif" value="1" @checked(old('actif', true)) class="w-4 h-4 rounded">
        ...
        <div class="flex gap-3 pt-4">
            <a href="{{ route('sla.index') }}" class="px-6 py-2 border border-dark-700 rounded-lg text-white hover:bg-dark-800 transition">
                Annuler
            </a>
